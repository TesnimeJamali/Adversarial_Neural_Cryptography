...
\begin{tabular}{@{} l l @{\hspace{2cm}} l l @{}}
  ...
  \textbf{Sections}     & Abstract + Sections 1--12 \\[2pt]
...
via block-based chunking, structured image-patch encryption using
real image data, a configurable training step ratio between
...
\begin{tcolorbox}[contribbox, title={Keywords}]
  ...
  Image patch encryption $\cdot$ Loss function variants
\end{tcolorbox}
...
Section~\ref{sec:architecture} provides a detailed exposition of the system architecture,
including the Alice–Bob–Eve framework, the mix~\&~transform network
design, and the attention-enhanced variant.
Section~\ref{sec:loss} formally analyses the loss functions and optimisation
objectives, including the three loss function variants introduced in
the enhanced implementation.
Section~\ref{sec:training_dynamics} examines training dynamics, including convergence behaviour
and the saddle-point nature of the adversarial game.
Section~\ref{sec:message_modes} covers message complexity extensions: random binary, ASCII
text, and image-patch modes.
Section~\ref{sec:implementation} presents the implementation details with a full annotated
walkthrough of \code{main\_enhanced.py}.
Section~\ref{sec:security} evaluates the security properties of the learned
cryptosystem, including post-training \eve{} robustness evaluation.
Section~\ref{sec:results} presents and discusses experimental results
across all configurations: baseline random-binary mode, loss function
variants, training step ratios, the self-attention mechanism,
post-training robustness, ASCII text mode under both loss variants,
and structured image-patch training on CIFAR-10 data.
Section~\ref{sec:discussion} provides a critical discussion of the approach's strengths,
...
adversary. This gap is discussed in detail in Section~\ref{sec:security}.
...
leakage). This is formalised in Section~\ref{sec:loss}.
...
\begin{itemize}[leftmargin=1.8em, itemsep=4pt]
  ...
  \item \textbf{MSB bias}: printable ASCII codes 32--126 always have
        ...
        genuine finding confirmed experimentally in Section~\ref{subsec:ascii_results}.
...
(Section~\ref{subsec:ascii_results}) but does not affect Bob's decryption fidelity.
% =============================================================================
\newpage
...
This is essential for the experimental comparisons in Section~\ref{sec:results},
...
progress bar using Unicode block characters (\texttt{U+2588}, full block)
for immediate visual feedback during long training runs.
...
(Section~\ref{subsec:post_training_eve}) is repeated multiple times without retraining Alice and
...
\code{main\_enhanced.py} automatically generates two categories of
...
The $85\%$ security threshold (\code{0.85 $\times$ msg\_size / 2}) is separately drawn
as a dotted orange line, and is also reported in the console output of
\code{retrain\_eve\_robustness()} as the pass/fail criterion. Together, the two
plot types provide both an overall learning trajectory and a per-run security
verdict for every experimental configuration.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		% ── Mode branches ───────────────────────────────────────────────────────────
		% ASCII: goes to demo then retrain
		\node[sideblock, left=2.2cm of mode]  (asc)    {ASCII\\text demo};
		% Random: goes to demo then retrain
		\node[sideblock, right=2.2cm of mode] (rnd)    {Random binary\\demo};
		% Image patches: DIRECT to retrain (skips demo)
		\node[sideblock, below=0.6cm of mode] (img)    {Image patches\\skip demo};
		% ── Continuation after demo ──────────────────────────────────────────────────
		\node[block,    below=2.2cm of mode]  (retrain){\textbf{9.} Post-training Eve robustness evaluation};
		...
		% ASCII branch: mode -> demo -> retrain
		\draw[sarr] (mode.west)  -- (asc.east)
		node[midway, above, font=\tiny\itshape]{ascii};
		\draw[sarr] (asc.south)  |- (retrain.west);
		% Random branch: mode -> demo -> retrain
		\draw[sarr] (mode.east)  -- (rnd.west)
		node[midway, above, font=\tiny\itshape]{random};
		\draw[sarr] (rnd.south)  |- (retrain.east);
		% Image patches branch: mode DIRECTLY to retrain (skips demo)
		\draw[sarr] (mode.south) -- (img.north)
		node[midway, right, font=\tiny\itshape]{image\_patches};
		\draw[sarr] (img.south)  -- (retrain.north);
	\end{tikzpicture}
	\caption{Complete execution flow of \code{main\_enhanced.py}.
		...
		The \texttt{image\_patches} mode bypasses the text demo and connects
		directly to post-training evaluation (step 9).}
	\label{fig:exec_flow}
\end{figure}
...
(Section~\ref{subsec:shannon}) requires:
%
\begin{equation}
  H(P \mid C) = H(P)
...
post-training Eve robustness evaluation described in Section~\ref{subsec:post_training_eve}.
...
In the ASCII chunking mode (Section~\ref{sec:message_modes}), the same key $K$ is
...
the effects of each configurable parameter (loss function variant,
training step ratio, and attention mechanism), and the results for
ASCII text mode under both quadratic and linear loss variants.
The section concludes with an analysis of the image-patch training
experiment, which tests whether the learned cipher generalises to
...
Across the seeds tested (1, 42, and 13), the system exhibited a mix of
...
good training convergence and strong post-training robustness (5/5 secure
...
theoretically in Section~\ref{subsec:four_phases}.
...
Eve error of $14.08$ bits, which appears superior at first glance. However, this is a
degenerate outcome. Eve's \emph{training-time} error of $14.56$ bits
...
that has not co-adapted makes $\sim$14 errors rather than $\sim$8
...
\subsubsection*{Discussion: \texttt{eve\_steps=2} is the Sweet Spot}

The paper's recommendation of \code{eve\_steps=2} is confirmed by these results.
The baseline loss curve and robustness chart for \code{eve\_steps=2} are shown
in Figures~\ref{fig:loss_curve_baseline} and~\ref{fig:eve_retrain} respectively
(Section~\ref{subsec:loss_curves} and~\ref{subsec:robustness_results_sec10}).
It achieves the \textbf{best robustness} of any ratio: 5/5 secure runs and
mean $9.04$ bits ($113\%$ of baseline). \code{Eve\_steps=3} also achieves 5/5 secure
but with slightly lower mean robustness ($8.42$ bits). \code{Eve\_steps=1} fails
...
Counter-intuitively, \code{eve\_steps=4} also fails (0/5 secure, mean $6.06$ bits):
...
non-monotonic, with \code{eve\_steps=2} occupying the optimal balance.
% =============================================================================
...
The post-training Eve robustness evaluation (Section~\ref{subsec:post_training_eve}) provides the
...
Section~\ref{subsec:mode_ascii} ($N=16$, seed~13, quadratic loss, eve\_steps=2, 20{,}000 steps).
...
anti-correlation phenomenon observed in random binary mode (Section~\ref{subsec:loss_variant_results})
...
\begin{figure}[H]
	...
	\caption{Training loss curves for ASCII text mode with linear loss variant
		...
		pattern as the random + linear case (Section~\ref{subsec:loss_variant_results}).}
...
% =============================================================================
\subsection{Image Patches Training: Structured Visual Data}
\label{subsec:image_patches_results}
% =============================================================================

\subsubsection*{Motivation}

The experiments in Sections~\ref{subsec:baseline_results}--\ref{subsec:ascii_linear_results}
trained exclusively on messages drawn from uniform or near-uniform binary distributions.
A natural question is whether the learned cipher \emph{generalises to structured,
	real-world data} whose statistics differ substantially from a uniform random source.
Image patches are an ideal testbed: natural image patches exhibit strong spatial
correlations (adjacent pixels tend to have similar values), a highly non-uniform
marginal distribution (most 4$\times$4 patches are near-uniform grey or contain a
single edge), and a known public distribution (CIFAR-10), making the task
harder than encrypting random bits. A successful result would also directly
validate the \code{ImageEncryptor} pipeline described in
Sections~\ref{sec:implementation} and~\ref{subsec:mode_ascii}.
...
\textbf{Dataset.}
The training corpus consists of all 50{,}000 CIFAR-10 training images converted
to grayscale and cached in \code{./training\_images/}. During training, the
\code{ImagePatchLoader} samples random 4$\times$4 patches from random images
at each step. Each patch is binarised by thresholding at 128 (pixel $> 127
\to +1$, else $-1$), producing a 16-bit message vector in $\{-1,+1\}^{16}$.
This binarisation matches the protocol used by \code{image\_encryptor.py}
for downstream image encryption.

\textbf{Architecture.}
The standard \code{CipherNet} / \code{AttackerNet} architecture
(Section~\ref{subsec:mix_transform}) is used without modification.
The message size ($N = 16$) matches the 4$\times$4 patch size exactly,
so no architectural change is required.

\textbf{Key hyperparameter difference: \code{eve\_steps = 4}.}
The random-mode baseline used \code{eve\_steps = 2}.
For image patches, \code{eve\_steps = 4} was chosen because the
non-uniform distribution of natural image patches provides more exploitable
statistical structure: an Eve trained on correlated patches can in principle
leverage spatial statistics that a purely random-guessing strategy would miss.
Increasing Eve's training pressure compensates for this advantage.
Section~\ref{subsec:step_ratio_results} showed that \code{eve\_steps = 4}
fails for the random baseline (Bob stalls at $1.84$ bits), but the
structured training signal of image patches changes the dynamics sufficiently
that a stronger adversary is necessary.

Table~\ref{tab:image_patches_params} summarises all hyperparameters.

\begin{table}[H]
	\centering
	\caption{Hyperparameters for the image patches training experiment.
		Differences from the random binary baseline are highlighted.}
	\label{tab:image_patches_params}
	\renewcommand{\arraystretch}{1.35}
	\begin{tabularx}{\linewidth}{@{} l c X @{}}
		\toprule
		\textbf{Parameter} & \textbf{Value} & \textbf{Rationale} \\
		\midrule
		\code{mode}       & \code{image\_patches} &
		Real image structure instead of uniform random bits \\
		\code{patch\_size}& $4$  &
		$4 \times 4$ pixels $= 16$ bits, matching $N = 16$ \\
		\code{msg\_size}  & $16$ &
		Original architecture parameter \\
		\code{key\_size}  & $16$ &
		Original architecture parameter \\
		\code{batch\_size}& $4096$ &
		Large batches for stable adversarial gradient estimates \\
		\code{steps}      & $100{,}000$ &
		\textbf{5$\times$ longer} than random mode; structured patches
		require more time for the adversarial dynamic to stabilise \\
		\code{eve\_steps} & $4$ &
		\textbf{Stronger than baseline ($2$)}; needed to exploit
		spatial statistics of natural images \\
		\code{loss\_fn}   & \code{quadratic} &
		Only variant producing stable convergence (Sections~\ref{subsec:loss_variant_results}) \\
		\code{seed}       & $13$ &
		Reproducibility (same as all other experiments) \\
		Dataset & CIFAR-10 grayscale & 50{,}000 images, $32 \times 32$ px each \\
		\bottomrule
	\end{tabularx}
\end{table}
...
Table~\ref{tab:image_patches_metrics} reports the terminal training metrics
at step~100{,}000.

\begin{table}[H]
	\centering
	\caption{Terminal training metrics for the image patches experiment
		(step~100{,}000, \code{eve\_steps=4}, quadratic loss, seed~13).
		Random binary baseline (Section~\ref{subsec:baseline_results})
		shown for comparison.}
	\label{tab:image_patches_metrics}
	\renewcommand{\arraystretch}{1.4}
	\begin{tabularx}{\linewidth}{@{} l X X @{}}
		\toprule
		\textbf{Metric} & \textbf{Image Patches} & \textbf{Random Baseline} \\
		\midrule
		Bob reconstruction error  & $2.19$ bits / $16$  ($86.3\%$ correct) &
		$0.56$ bits / $16$ \\
		Eve reconstruction error (training) & $3.17$ bits / $16$  ($80.2\%$ correct) &
		$7.62$ bits / $16$ \\
		Composite loss $\mathcal{L}_{AB}$   & $0.7483$ & $\approx 0.140$ \\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{../plots/image_patches_4x4/training_loss_curve.png}
	\caption{Training loss curves for the image patches experiment (seed~13,
		\code{eve\_steps=4}, quadratic loss, 100{,}000 steps).
		Bob's error (red) and Eve's error (green) are both substantially
		elevated compared to the random binary baseline: Bob stabilises
		near $2.19$ bits and Eve near $3.17$ bits, with persistent
		oscillation throughout training.
		Unlike the random mode (Figure~\ref{fig:loss_curve_baseline}),
		neither curve achieves clean convergence within the training budget.}
	\label{fig:image_patches_loss}
\end{figure}

\subsubsection*{Post-Training Eve Robustness}

\begin{table}[H]
	\centering
	\caption{Post-training Eve robustness results for the image patches
		experiment ($5$ runs $\times$ $5{,}000$ steps).
		Threshold for ``Secure'': $\geq 0.85 \times 8.0 = 6.8$ bits.}
	\label{tab:image_patches_robustness}
	\renewcommand{\arraystretch}{1.35}
	\begin{tabular}{@{} c c c c @{}}
		\toprule
		\textbf{Run} & \textbf{Best Eve error (bits)} &
		\textbf{vs.\ baseline ($N/2 = 8$)} & \textbf{Status} \\
		\midrule
		1 & $13.551$ & $169.4\%$ & \textcolor{GreenOK}{Secure} \\
		2 & $13.502$ & $168.8\%$ & \textcolor{GreenOK}{Secure} \\
		3 & $13.678$ & $170.9\%$ & \textcolor{GreenOK}{Secure} \\
		4 & $13.654$ & $170.7\%$ & \textcolor{GreenOK}{Secure} \\
		5 & $13.604$ & $170.1\%$ & \textcolor{GreenOK}{Secure} \\
		\midrule
		\textbf{Mean} & $\mathbf{13.598}$ & $\mathbf{169.9\%}$ &
		\textbf{5/5 Secure} \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{../plots/image_patches_4x4/eve_retrain_results.png}
	\caption{Post-training Eve robustness bar chart for the image patches
		experiment. All five bars are green and far above both the
		random baseline ($8.0$ bits, dashed) and the security threshold
		($6.8$ bits, dotted). Mean $13.598$ bits ($169.9\%$ of baseline).
		The elevated values are consistent with the anti-correlation
		phenomenon discussed below.}
	\label{fig:image_patches_retrain}
\end{figure}

% ── 10.8.4 Discussion ────────────────────────────────────────────────────────

\subsubsection*{Discussion: The Training-Time vs.\ Post-Training Anomaly}

The most striking feature of the image patches results is the large
\textbf{discrepancy between training-time and post-training Eve error}:
training-time Eve achieved only $3.17$ bits error (well below the $8.0$
random baseline, suggesting she was actively decoding the ciphertext with
$\approx$80\% bit accuracy), yet all five post-training fresh Eves achieve
$\approx$13.6 bits --- far \emph{above} the random baseline. This
pattern strongly resembles the \textbf{anti-correlation collapse} identified
for the linear loss variant in Section~\ref{subsec:loss_variant_results},
where an apparent ``high security'' post-training score masked a degenerate
cipher that could be trivially broken by negating Eve's output.

Several mechanisms may be responsible, and they are not mutually exclusive:

\paragraph{Mechanism 1: Anti-correlation driven by \texttt{eve\_steps=4}.}
Section~\ref{subsec:step_ratio_results} established that \code{eve\_steps=4}
causes training failure in the random binary mode, with Bob stalling at
$1.84$ bits. The image patches result is consistent with the same
pathology: excessive adversarial pressure causes Alice to learn a systematically
\emph{inverted} cipher in the image domain. The co-trained Eve adapts to this
inversion (hence her low training error of $3.17$ bits), but a fresh Eve
initialised randomly has not learned the inversion convention --- making
$13.6$ bits her effective error. An adversary who simply negated her output
would achieve approximately $16 - 13.6 = 2.4$ bits error, which constitutes
a near-complete break. This interpretation is supported by the remarkably
tight clustering of the five post-training runs (range $13.50$--$13.68$ bits,
standard deviation $< 0.08$ bits), which is the same signature of
a \emph{deterministic structural property} observed for the ASCII + linear
variant (Section~\ref{subsec:ascii_linear_results}, range $14.99$--$15.02$ bits).

\paragraph{Mechanism 2: Exploitation of the patch binarisation bias.}
Natural CIFAR-10 patches, when binarised at 128, are heavily biased:
patches from sky, background, or near-uniform regions produce mostly
$+1$ bits, while darker patches produce mostly $-1$ bits. This
distributional asymmetry (unlike the perfectly uniform $\{-1,+1\}$
distribution of random mode) provides Alice with structural regularities
to exploit when constructing an anti-correlated cipher. The effect is
analogous to the MSB bias identified for ASCII mode
(Section~\ref{subsec:ascii_results}), but more pronounced because the
full 16-bit patch distribution is far from uniform.

\paragraph{Mechanism 3: Co-trained Eve overfitting to Alice's weight trajectory.}
A subtler possibility is that the co-trained Eve, over 100{,}000 steps, adapted
to the specific sequence of Alice's weight updates rather than to Alice's final
cipher alone. The fresh post-training Eve sees only the \emph{terminal} cipher
and must learn from scratch, potentially encountering a different effective
loss landscape than the co-trained Eve experienced. This mechanism would
predict higher fresh-Eve error even without anti-correlation, but cannot alone
explain the $>1.5\times$ baseline overshoot.

\paragraph{Net assessment.}
The most parsimonious interpretation, consistent with all three mechanisms,
is that the image patches experiment produced an \textbf{anti-correlated
	cipher} driven by excessive adversarial pressure (\code{eve\_steps=4}),
amplified by the non-uniform patch distribution. The 5/5 ``secure''
classification is therefore misleading in the same way as the linear
loss variant results: an informed adversary negating Eve's output would
achieve $\approx$2.4 bit error, not 13.6. The appropriate corrected
security metric --- analogous to the one applied to the linear variant ---
would classify this run as a failure.

Table~\ref{tab:image_patches_comparison} summarises the comparison of the
image patches experiment against all other evaluated configurations.

\begin{table}[H]
	\centering
	\caption{Image patches results compared to other experimental configurations.
		$\dagger$~Anti-correlation suspected; negating Eve's output
		gives $\approx 2.4$ bits error. Genuine security would require
		post-training Eve error in $[6.8, \sim 10]$ bits.}
	\label{tab:image_patches_comparison}
	\renewcommand{\arraystretch}{1.4}
	\begin{tabularx}{\linewidth}{@{} l c c c c @{}}
		\toprule
		\textbf{Configuration} & \textbf{Bob err.} &
		\textbf{Eve (train)} & \textbf{Mean robust.} & \textbf{Secure} \\
		\midrule
		Random, quad, $e=2$ (baseline) &
		$0.56$ & $7.62$ & $9.04$ & $5/5$ \\
		Random, linear, $e=2$ &
		$0.01$ & $14.56$ & $14.08$ & $5/5^\dagger$ \\
		ASCII, quad, $e=2$ &
		$0.04$ & $7.32$  & $6.20$ & $2/5$ \\
		ASCII, linear, $e=2$ &
		$0.47$ & $12.49$ & $15.01$ & $5/5^\dagger$ \\
		\textbf{Image patches, quad, $e=4$} &
		$\mathbf{2.19}$ & $\mathbf{3.17}$ & $\mathbf{13.60}$ & $\mathbf{5/5}^\dagger$ \\
		\bottomrule
	\end{tabularx}
\end{table}

\subsubsection*{What Worked and What Did Not}

\textbf{What worked.}
All five post-training retraining runs classified as nominally secure, confirming
that the final cipher does not yield to a standard gradient-based neural attacker
initialised from scratch. The extended training budget (100{,}000 steps) and
large batch size (4{,}096) prevented the training from diverging entirely ---
Bob's error, while elevated, remained below $N/2$, indicating at least partial
reconstruction fidelity throughout.

\textbf{What did not work.}
Bob's reconstruction error of $2.19$ bits ($13.7\%$ of bits wrong) falls far
short of the near-zero errors achieved in random and ASCII modes. At this error
rate, a 4$\times$4 patch decoded by Bob would contain on average 2--3 incorrect
pixels, producing visibly degraded image reconstructions when assembled into full
images via the \code{ImageEncryptor} pipeline. The likely cause is the combination
of a stronger adversary (\code{eve\_steps=4}) and the structured input distribution,
which together prevent Alice and Bob from finding a stable, genuinely key-dependent
encryption strategy within the training budget.

\subsubsection*{Directions for Improvement}

\begin{enumerate}[leftmargin=2em, itemsep=4pt]
	\item \textbf{Reduce \code{eve\_steps} to 2 or 3.} The random mode
	analysis (Section~\ref{subsec:step_ratio_results}) showed that
	\code{eve\_steps=2} is the sweet spot. For image patches,
	\code{eve\_steps=3} may provide sufficient adversarial pressure
	without triggering the anti-correlation collapse.
	\item \textbf{Soft binarisation or direct pixel training.} Binarising
	patches at a fixed threshold discards greyscale information. Training
	directly on continuous $[-1,+1]$ normalised pixel values
	(rather than hard-thresholded bits) would give Alice and Bob a richer
	signal and reduce the distributional bias identified in Mechanism~2.
	\item \textbf{Loss scaling for structured inputs.} A structured patch
	distribution may benefit from a reweighted loss that penalises Bob's
	errors on informative (high-variance) patches more heavily than on
	near-uniform patches, stabilising the reconstruction term
	$\mathcal{L}_B$ against the adversarial penalty.
	\item \textbf{Curriculum learning.} Begin training on random binary
	messages (as in the baseline) and gradually introduce image patches
	as training progresses. This warm-start strategy allows Alice and Bob
	to establish a key-dependent cipher on easy data before encountering
	the harder structured distribution.
\end{enumerate}
...
As demonstrated in Section~\ref{subsec:ascii_results}, the system operates without
...
As established in Section~\ref{subsec:formal_security}, the system provides no formal security
...
\begin{enumerate}[leftmargin=2em, itemsep=6pt]
  ...
  \item \textbf{Loss function sensitivity}: the comparison of quadratic,
        linear, and BCE variants (Section~\ref{subsec:loss_variant_results}) confirms the original
  ...
  \item \textbf{Step ratio sensitivity}: the paper's recommendation of
        eve\_steps=2 is \emph{confirmed} by our experiments (Section~\ref{subsec:step_ratio_results}).
  ...
  \item \textbf{Attention's effect at $N=16$}: contrary to the theoretical
        expectation, the self-attention mechanism \emph{degrades} performance at
        $N=16$, reducing success rate from $5/5$ to $0/5$ secure runs and
        increasing Bob's error by $2.8\times$ (from $0.56$ to $1.55$ bits).
        At this small sequence length, the convolutional architecture already
        has sufficient capacity; the attention layer adds parameters without
        useful inductive bias, making optimisation harder. Benefits of attention
        are nonetheless expected at larger $N$ where fixed convolutional
        receptive fields become a genuine bottleneck for learning long-range
        bit dependencies (Section~\ref{subsec:attention_results}).

  \item \textbf{Distribution invariance}: the successful operation of
        the cipher in ASCII text mode (Section~\ref{subsec:ascii_results}),
  ...
  \item \textbf{Post-training robustness as a necessary test}: the
        ...
        The image-patch experiment makes this point compellingly: a
        training-time Eve error of only $3.17$ bits coexists with a
        post-training error of $\approx 13.6$ bits, a discrepancy
        traceable to anti-correlation rather than genuine cipher strength.

  \item \textbf{Distribution sensitivity}: structured image patches
        (CIFAR-10) break the clean adversarial dynamic observed for
        uniform random and ASCII inputs. The non-uniform marginal
        distribution of binarised natural patches, combined with the
        necessary increase in adversarial pressure (\code{eve\_steps=4}),
        leads to an anti-correlated cipher instead of a key-dependent one.
        This reveals that the system's robustness is partly a function of
        training-distribution uniformity, and that real-world data requires
        additional measures such as soft binarisation or curriculum learning
...
using neural networks as cryptanalysis tools. Gohr (2019)
demonstrated that neural networks can learn to distinguish
partially decrypted ciphertext from random data, achieving
state-of-the-art results on reduced-round Speck, a lightweight
block cipher~\cite{gohr2019improving}.
This line of work underscores that the adversarial role of \eve{}
in the present system is not merely academic --- neural network
attackers are now a genuine threat in classical cryptanalysis as well.
...
\begin{enumerate}[leftmargin=2em, itemsep=6pt]
  ...
  \item \textbf{CBC/CTR mode analogues}: implementing a neural analogue
        ...
        weakness identified in Section~\ref{subsec:mode_variable} and bring the system closer
...
\begin{enumerate}[leftmargin=2em, itemsep=5pt]
  ...
  \item \textbf{Architecture}: the mix~\&~transform design (FC + 4
        ...
        enough (567 parameters for Alice/Bob, 311 for Eve) for adversarial
        training to find meaningful solutions. The optional self-attention
        layer extends this expressivity at the cost of modest additional
        complexity.
  ...
  \item \textbf{Training dynamics}: successful convergence follows a
        reproducible four-phase trajectory. The training step ratio exhibits
        non-monotonic behaviour: \code{eve\_steps=1} fails (0/5 secure),
        \code{eve\_steps=2} achieves the best robustness (5/5 secure,
        mean $9.04$ bits), \code{eve\_steps=3} also succeeds but with
        slightly lower robustness (mean $8.42$ bits), and
        \code{eve\_steps=4} paradoxically fails again (0/5 secure,
        mean $6.06$ bits) due to excessive adversarial pressure preventing
        convergence within the 20{,}000-step budget. Non-monotonic
        loss curves are expected throughout and should not trigger
        early stopping.
  ...
  \item \textbf{Message complexity}: the learned cipher generalises
        ...
        using the same network, loss, and training procedure. Structured
        image patches (CIFAR-10 grayscale) pose a harder challenge:
        the non-uniform patch distribution and necessary increase in
        adversarial pressure (\code{eve\_steps=4}) together produce an
        anti-correlated cipher rather than genuine key-dependent encryption,
        pointing to the need for distribution-aware training strategies.
...
A.~Gohr,
\newblock ``Improving attacks on round-reduced Speck32/64 using deep learning,''
\newblock in \textit{Advances in Cryptology --- CRYPTO 2019},
  Lecture Notes in Computer Science, vol.~11693, pp.~150--179, Springer, 2019.
